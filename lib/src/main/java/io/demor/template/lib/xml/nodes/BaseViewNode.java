...
import io.demor.template.lib.xml.XmlTags;
...
public abstract class BaseViewNode extends XmlNode {
	...
	protected void onFinishMapAttr() {
		...
		try {
			unit = calcUnit(width);
			if (unit != null) {
				mWidthInfo = new Pair<>(unit, unit.getData(width));
			}
		}catch (Throwable e){
            e.printStackTrace();
        }
		...
		try {
			...
			if (unit != null) {
				mHeightInfo = new Pair<>(unit, unit.getData(height));
			}
		}catch (Throwable e) {
            e.printStackTrace();
        }
	...
	public interface ICalculateMeasuringUnit{
