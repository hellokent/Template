...
import static io.demor.template.lib.Utils.Reflect.getLocalNameFromField;
import static io.demor.template.lib.Utils.Reflect.isSubclassOf;
import static io.demor.template.lib.Utils.Reflect.setField;
...
public class XmlNode implements Cloneable{
	...
	private static HashMap<Class<?>, XmlNode> XML_NODE_CACHE = new HashMap<>();
    ...
    private ArrayList<XmlNode> mChildren = new ArrayList<>();
	...
	private static String toFieldMapKey(final Field field){
        ...
        if (xml == null
                || (!isSubclassOf(type, String.class)
                    && !isSubclassOf(type, Collection.class)
                    && !isSubclassOf(type, XmlNode.class))
                || (TextUtils.isEmpty(xml.value()) && isSubclassOf(type, String.class))){
			return null;
		}

        final String localName = getLocalNameFromField(field);
	...
	public static XmlNode newXMLNodeInstance(Class<?> nodeClass){
        ...
        if (!isSubclassOf(nodeClass, XmlNode.class)){
            return null;
        }
		...
		XmlNode result = XML_NODE_CACHE.get(nodeClass);
		if (result != null){
			result = result.clone();
			...
			try {
				...
				XML_NODE_CACHE.put(nodeClass, result.clone());
			} catch (Exception e) {
				e.printStackTrace();
			}
			++sNewCount;
    ...
    private void toText2(final XmlSerializer serializer) throws IOException, IllegalAccessException {
        ...
        LinkedList<XmlNode> childNodes = new LinkedList<>();
		for (Map.Entry<String, Field> entry: getFieldMap().entrySet()){
            if (entry.getValue().get(this) == null){
                continue;
            }
	...
	protected final XmlNode getChildNodeInstance(String childLocalName){
        ...
        L.v("node(%s) add child(%s)", getClass().getSimpleName(), result == null ? "null" : result.getClass().getSimpleName());
